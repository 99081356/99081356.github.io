...
def key_word_check():
    ...
    if form.validate_on_submit():
        ...
        no_matches = []
        message = []
        # 拿key_words去匹配matches，如果匹配到把key_word从matchess中删除，否则直接添加key_word到no_results中。
        for key_word in key_words:
            if key_word:
                if key_word in content_matches:
                    content_matches.remove(key_word)
                else:
                    no_matches.append(key_word)
        if content_matches and no_matches:
            message.append(
                f"结论：说明书和附图中的图号无法一一对应，说明书中出现而附图未出现的图为:'{content_matches}'!<br>附图中出现而说明书中未出现的图为：'{no_matches}'！")
        elif content_matches:
            message.append(f"结论：说明书和附图中的图号无法一一对应，说明书中出现而附图未出现的图为:'{content_matches}'！")
        elif no_matches:
            message.append(f"结论：说明书和附图中的图号无法一一对应，附图中出现而说明书中未出现的图为：'{no_matches}'！")
        else:
...
#             #     # 返回首页模板，并传递消息
#             #     return render_template('keyword_check_tool.html', form=form, message='No file part')
#             # 如果文件名称为空
#             if file.name == '':
#                 # 返回首页模板，并传递消息
#                 return render_template('keyword_check_tool.html', form=form, message='未上传文件！')
#             # 如果文件存在
#             if file:
#                 # 获取要搜索的图表号
...
#                     # 返回首页模板，并传递消息
#                     return render_template('keyword_check_tool.html', form=form, message='请输入需要检索的词！')
#                 # 创建文档对象
#                 doc = Document(file)
#                 message = []
#                 matches = []
...
#                     # 返回首页模板，并传递消息
#                 return render_template('keyword_check_tool.html', form=form, message=message, details=details,
#                                        title='常用工具')
#     return render_template('keyword_check_tool.html', form=form, title='常用工具')
